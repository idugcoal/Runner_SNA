...
class SelectStopsSterile extends Component {
  ...
  renderEndingButton() {
    if (this.props.runType === 'departure') {
      return (
        <Button onPress={this.onGateArrival.bind(this)}>Gate Arrival</Button>
      )
      ...
      <View style={Style.container}>
        <CardSection>
          <View style={Style.row}>
            <Button onPress={() => this.setState({ terminal: terminalA })}>
              Terminal A
            </Button>
            <Button onPress={() => this.setState({ terminal: terminalB })}>
              Terminal B
            </Button>
            <Button onPress={() => this.setState({ terminal: terminalC })}>
              Terminal C
            </Button>
          </View>
        </CardSection>
        <View style={Style.content}>{this.renderButtons()}</View>
        <CardSection>{this.renderEndingButton()}</CardSection>
        <Footer />
        <Modal
          ...
          <View>
            <Text style={{ fontSize: 20, marginBottom: 10 }}>
              Are you at Gate #{this.props.destinationGate}?
            </Text>
            <CardSection>
              <View style={Style.row}>
                <Button onPress={this.onGateSuccess.bind(this)}>Yes</Button>
                <Button onPress={this.onGateFail.bind(this)}>No</Button>
              </View>
            </CardSection>
          </View>
        </Modal>
...
export default connect(mapStateToProps, { setTimeEnd, addStop })(
  SelectStopsSterile,
)
